...
table_name = "extreme_events_yearly_summary"
...
# reverse coldwave values since coldest coldwave is the day with lowest temperature
events = events.withColumn(
    "sort_value",
    when(col("event_type") == "coldwave", -col("value"))
    .otherwise(col("value"))
)
window = (
    Window
    .partitionBy("station_id", "year", "event_type")
    .orderBy(col("sort_value").desc())
)
