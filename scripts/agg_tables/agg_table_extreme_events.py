...
extreme = (
    ...
    .withColumn(
        "heavy_snow",
        when((col("element") == "SNWD") & (col("value") >= snwd_threshold), col("value").cast("double"))
    )
    # PRECIP
    .withColumn(
        "heavy_precip",
        when((col("element") == "PRCP") & (col("value") >= precp_threshold), col("value")/10.0)
    )
    # HEATWAVE threshold
    .withColumn(
        "heatwave_temp",
        when((col("element") == "TMAX") & (col("value") > tmax_threshold), col("value")/10.0)
    )
    # COLDWAVE threshold
    .withColumn(
        "coldwave_temp",
        when((col("element") == "TMIN") & (col("value") < tmin_threshold), col("value")/10.0)
    )
)
